...
def svm_test(X, y, test_sizes=(0.2, 0.4, 0.6, 0.8, 0.9, 0.95, 0.99), repeat=10):
    ...
    for test_size in test_sizes:
        ...
        for i in range(repeat):#遍历重复次数
            ...
            y_score = svm._predict_proba_lr(X_test)
        ...
        result_roc_auc_list.append((np.mean(roc_auc_list), np.std(roc_auc_list)))
    return result_macro_f1_list, result_micro_f1_list, result_roc_auc_list#返回的是均值和标准差的列表


def evaluate_results_nc(embeddings, labels, num_classes):#输入的是测试集的嵌入，测试集的标签，类别的数量
    repeat = 20
    print('SVM test')
    svm_macro_f1_list, svm_micro_f1_list, svm_roc_auc_list = svm_test(embeddings, labels, repeat=repeat)#经过SVM进行测试20次，返回每次的评价列表
    #打印训练集比率为？？下的结果
    ...
    print('Macro-F1: ' + ', '.join(['{:.4f}~{:.4f}({:.2f})'.format(macro_f1_mean, macro_f1_std, train_size) for
                                    (macro_f1_mean, macro_f1_std), train_size in
                                    zip(svm_macro_f1_list, [0.2, 0.4, 0.6])]))
    print('Micro-F1: ' + ', '.join(['{:.4f}~{:.4f}({:.2f})'.format(micro_f1_mean, micro_f1_std, train_size) for
                                    (micro_f1_mean, micro_f1_std), train_size in
                                    zip(svm_micro_f1_list, [0.2, 0.4, 0.6])]))
    print('Roc Auc Score: ' + ', '.join(['{:.4f}~{:.4f}({:.2f})'.format(roc_auc_mean, roc_auc_std, train_size) for
                                    (roc_auc_mean, roc_auc_std), train_size in
                                    zip(svm_roc_auc_list, [0.2, 0.4, 0.6])]))
